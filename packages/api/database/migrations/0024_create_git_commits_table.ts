  ...
  async up() {
    this.schema.createTable(this.tableName, (table) => {
      ...
      table
        ...
        .onDelete('CASCADE')
      table.varchar('sha', 40).notNullable()
      table.text('message').notNullable()
      table.text('diff_summary').nullable()
      table.varchar('author_name', 255).notNullable()
      table.varchar('author_email', 255).notNullable()
      table.timestamp('committed_at').notNullable()
      table.varchar('branch', 255).notNullable()
