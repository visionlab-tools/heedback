<script lang="ts">
  ...
  import { createArticlePageState } from './page-state.svelte'
...
<div class="max-w-3xl mx-auto px-4 py-12">
  ...
  <article class="mt-6">
    ...
    {#if data.article.author}
      <p class="mt-2 text-sm text-gray-500">
        {$_('help.by_author', { values: { author: data.article.author.name } })}
  ...
  <div class="mt-12 pt-8 border-t border-gray-200">
    {#if state.feedbackSent}
      <p class="text-sm text-gray-500">{$_('help.thanks_feedback')}</p>
    {:else}
      <p class="text-sm font-medium text-gray-700">{$_('help.was_helpful')}</p>
