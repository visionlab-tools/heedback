<script lang="ts">
  ...
  import { createFeedbackPostState } from './page-state.svelte'
...
<svelte:head>
  <title>{data.post.title} - {$_('feedback.title')}</title>
</svelte:head>
...
<div class="max-w-3xl mx-auto px-4 py-12">
  ...
  <div class="mt-6 flex items-start gap-6">
    <div class="flex flex-col items-center min-w-[56px] bg-gray-50 rounded-xl py-3 px-2">
      ...
      <span class="text-xs text-gray-400">{$_('feedback.votes', { values: { count: '' } }).replace(' ', '')}</span>
    </div>
    ...
    <div class="flex-1">
      ...
      <p class="mt-2 text-sm text-gray-500">
        {$_('feedback.by_author', { values: { author: data.post.author?.name || 'Anonymous' } })}
      </p>
  ...
  <div class="mt-12">
    <h2 class="text-lg font-semibold text-gray-900">{$_('feedback.comments', { values: { count: data.comments.length } })}</h2>
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-200">
          <div class="text-sm">
            ...
            {#if comment.author?.type === 'admin'}
              <span class="ml-1 text-xs bg-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded">{$_('feedback.team')}</span>
            {/if}
          </div>
    ...
    {#if data.comments.length === 0}
      <p class="mt-4 text-sm text-gray-400">{$_('feedback.no_comments')}</p>
    {/if}
  </div>
</div>
