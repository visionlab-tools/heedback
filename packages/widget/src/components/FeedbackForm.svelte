...
<form onsubmit={state.handleSubmit}>
  ...
  {#if boards.length > 1}
    <div style="margin-bottom: 12px;">
      <label style="display: block; font-size: 12px; font-weight: 500; color: #374151; margin-bottom: 4px;">{t(locale, 'feedback.category')}</label>
      <select
  ...
  <div style="margin-bottom: 12px;">
    <label style="display: block; font-size: 12px; font-weight: 500; color: #374151; margin-bottom: 4px;">{t(locale, 'feedback.title')}</label>
    <input
      type="text"
      bind:value={state.title}
      oninput={state.handleTitleInput}
      required
      placeholder={t(locale, 'feedback.title_placeholder')}
  ...
  {#if state.similarPosts.length > 0}
    <div style="margin-bottom: 12px; padding: 10px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px;">
      <p style="font-size: 11px; color: #92400e; font-weight: 600; margin: 0 0 6px;">{t(locale, 'feedback.similar')}</p>
  ...
  <div style="margin-bottom: 16px;">
    <label style="display: block; font-size: 12px; font-weight: 500; color: #374151; margin-bottom: 4px;">{t(locale, 'feedback.description')}</label>
    <textarea
      bind:value={state.body}
      rows={4}
      placeholder={t(locale, 'feedback.description_placeholder')}
  ...
  <button
    ...
    "
  >
    {state.submitting ? t(locale, 'feedback.submitting') : t(locale, 'feedback.submit')}
  </button>
</form>
